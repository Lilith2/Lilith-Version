...
namespace eft_dma_radar.Tarkov.EFTPlayer
{
    ...
    public abstract class Player : IWorldEntity, IMapEntity, IMouseoverEntity, IESPEntity, IPlayer
    {
        ...
        /// <summary>
        /// Rounds a number to the nearest multiple of 3, ensuring values between -2 and 2 inclusive return 0.
        /// </summary>
        private int RoundToMultipleOfThree(float number)
        {
            // If the absolute value is less than or equal to 2, return 0
            ...
            if (number > 0)
            {
                ...
                return remainder == 0 ? (int)number : (int)number + (3 - remainder);
            }
            // For negative numbers
            else
            {
                int remainder = (int)Math.Abs(number) % 3;
                if (remainder == 0)
                    return (int)number;
                else
                    return (int)number - (3 - remainder);
            }
        }
        ...
        {
            // Calculate the raw height difference
            var rawHeightDiff = playerHeight - localPlayerHeight;

            // Round to the nearest multiple of 3 (always rounding up for positive values, down for negative)
            int heightDiff = RoundToMultipleOfThree(rawHeightDiff);
            ...
            for (int i = 0; i < Math.Abs(heightDiff); i++)
            {
                if (i > 5)
                    break;
